...
class AdminAuth {
  ...
  initAuth() {
    ...
    if (!this.currentUser && !window.location.pathname.includes('admin-login')) {
...
function loadRecentProducts() {
  ...
  if (recent.length === 0) {
    recentProductsContainer.innerHTML = '<p class="empty-state"><i class="fas fa-inbox"></i> No products yet. <a href="admin-add-product.html">Add your first product</a></p>';
    return;
  }
...
function loadProductsTable() {
  ...
  if (products.length === 0) {
    tbody.innerHTML = '<tr class="empty-row"><td colspan="8"><div class="empty-state"><i class="fas fa-inbox"></i><p>No products found</p><a href="admin-add-product.html" class="btn btn-primary btn-sm">Add Product</a></div></td></tr>';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (path.includes('admin-login')) {
    ...
    initDashboard();
  } else if (path.includes('admin-products.html') && !path.includes('admin-add-product')) {
    initProductsPage();
